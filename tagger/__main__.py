import argparse, os.path, sys, tagger.core, tagger.error
...
def init(args):
    try:
        tagger.core.find_base_dir()
        if args.force:
            raise tagger.error.NotInTaggedDir(".")
        else:
            sys.stderr.write("WARNING: This directory is inside an " +
                    ...
                    + "here.\n")
    except tagger.error.NotInTaggedDir:
        tagger.core.initialize_base_dir()
        sys.stdout.write("Initialized tag tracking under the current directory.\n")

def tag(args):
    for tag in args.tags:
        try:
            tagger.core.tag_add_files(tag, args.files)
            sys.stdout.write("'{}' tag applied to all chosen files.\n".format(tag))
        except tagger.error.InvalidTag as e:
            sys.stderr.write("ERROR: " + str(e) + "\n")

def untag(args):
    for tag in args.tags:
        try:
            tagger.core.tag_remove_files(tag, args.files)
...
def tags(args):
    for filename in args.files:
        try:
            tags = tagger.core.file_get_tags(filename)
            if tags == []:
                ...
                for tag in tags:
                    sys.stdout.write("\t{}\n".format(tag))
                    sys.stdout.write("")
        except tagger.error.NotInTaggedDir as e:
            sys.stderr.write("ERROR: " + str(e) + "\n")

def files(args):
    for tag in args.tags:
        try:
            files = tagger.core.tag_get_files(tag, rel_paths=True)
            if files == []:
